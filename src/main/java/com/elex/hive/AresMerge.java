...
public class AresMerge {
	...
	public static int mergeData(String sql, String jarName, String []functionName) {
		...
		try {
			...
			for(int i = 0, len = functionName.length; i < len; i++){
				stmt.execute(functionName[i]);
			}
//			mark = stmt.execute(sql);
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		 ...
		 String []functionName ={"create TEMPORARY function timeframe as 'com.elex.hive.TimeLagUDF'", "create TEMPORARY function splitcampidudtf as 'com.elex.hive.SplitCampIDUDTF'"};
		...
		int result = mergeData(sql, jarPath, functionName);
